...
with tab2:
    ...
    # Additional filters
    km_driven = st.number_input("Kilometers Driven", min_value=0, max_value=500000, value=100000, step=500)
    num_owners = st.selectbox("Number of Owners", ["First Owner", "Second Owner", "Third Owner", "Fourth & Above Owner"])
    insurance_type = st.selectbox("Insurance Type", ["Comprehensive", "Third-Party"])
    city = st.selectbox(
        "City", 
        ["Bangalore", "Chennai", "Jaipur", "Hyderabad", "Delhi", "Kolkata"], 
        index=0  # Default to "Bangalore"
    )

    # Predict price
    if st.button("Predict Price"):
        try:
            # Create input DataFrame with default values for missing columns
            input_data = pd.DataFrame({
                "Year of Manufacture": [year],
                "Mileage (km/l)": [mileage],
                "Engine Capacity (L)": [engine / 1000],  # Convert from cc to L
                "Max Power (hp)": [power],
                "Seats Capacity": [seats],
                "Fuel Type": [fuel_type],
                "Transmission Type": [transmission],
                "Car Age (years)": [2024 - year],
                "Kilometers Driven": [km_driven],
                "Number of Owners": [num_owners],
                "Insurance Type": [insurance_type],
                "City": [city],
                "Year of Registration": [year],  # Default to Year of Manufacture
