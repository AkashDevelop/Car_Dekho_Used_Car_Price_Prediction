...
# Set the page config for the theme (this must be the first Streamlit command)
st.set_page_config(
...
# Custom CSS for Streamlit theme
page_bg_img = """
<style>
...
# Add a heading for the next section
st.markdown(
    """
...
with tab1:
    ...
    st_lottie(lottie_coding, speed=1, width=800, height=300, key="car_animation")

    # Add overlay text above or below the Lottie animation
    st.markdown(
    """
...
with tab2:
    ...
    if st.button("Predict Price"):
        try:
            # Add missing columns with placeholder values for prediction
            input_data = pd.DataFrame({
                "Year of Manufacture": [year],
                "Mileage (km/l)": [mileage],
                "Engine Capacity (cc)": [engine],
                "Max Power (hp)": [power],
    ...
    scatter_fig = px.scatter(
        ...
        y=y_axis,
        color="Fuel Type",
        hover_data=["Car Manufacturer", "Car Model"],  # Replaced "Model" with "Car Model"
